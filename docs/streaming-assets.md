...
BakingSheet includes `StreamingAssetsFileSystem` to support this. It is required to install
[BetterStreamingAssets](https://github.com/gwiazdorrr/BetterStreamingAssets) package to use.
...
## Reading from StreamingAssets
Below is example of using `StreamingAssetsFileSystem` with `JsonSheetConverter` at runtime.
Keep in mind that path is relative from `Assets/StreamingAssets`.
```csharp
